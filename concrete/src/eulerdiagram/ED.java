...
public abstract class ED<Item> implements Serializable {

	private static final long serialVersionUID = -7094496252892984729L;
	public static String newline = System.getProperty("line.separator");
	
	protected TreeSet<Ellipse> ellipses;
   ...
   public ED(){
	...
	public eulerdiagram.Item getItemAtPoint(PointXY p) {
		...
		while (itemIterator.hasNext()) {
			...
			if (Math.sqrt(( i.getPosition().x-p.getX())*(i.getPosition().x-p.getX())+(i.getPosition().y-p.getY())*(i.getPosition().y-p.getY()))<7.0){
				return i;
			}
		}	
		return null;
	...
	public Ellipse getClosestEllipseAtPoint(PointXY p){
		...
		while (i.hasNext()) {
			...
			if (ei.hasContourPoint(p) && (min == -1 || ei.distance(p) < min)){
				min = ei.distance(p);
				tmpmin = new Ellipse(ei);
			}
		}
		return tmpmin;
	}
	
	protected Area computeArea(Zone z) {
		Rectangle2D.Double outer = new Rectangle2D.Double(0,0,800,600);
	...
	public abstract boolean remove(Ellipse oldEllipse);
	
	public abstract boolean add(Ellipse oldEllipse);
	...
	public TreeSet<Ellipse> getEllipses() {
		return ellipses;
	}

	public void setEllipses(TreeSet<Ellipse> ellipses) {
